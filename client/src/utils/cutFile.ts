...
*/
export function createChunk(file: File, index: number, chunkSize: number): Promise<{ start: number, end: number, index: number, hash: string, blob: Blob, name: string }> {
    return new Promise((resolve) => {
        ...
        fileReader.onload = (e) => {  // 当文件读取完成时的回调函数
            if (e.target) {
...
export function cutFile(file: Array<{
    name: string;
    progress: number;
    ...
    paused: boolean;
    file: File | null;
}>, chunkSize: number = 5): Promise<Array<Array<{ start: number, end: number, index: number, hash: string, blob: Blob }>>> {
    return new Promise((resolve) => {
        // 把响应式数据转换为普通数据
        ...
        let result: Array<Array<{ start: number, end: number, index: number, hash: string, blob: Blob }>> = [];  // 存储结果的数组
        ...
        for (let i = 0; i < numberOfThreads; i++) {
            ...
            worker.postMessage({  // 向线程发送消息
                chunkArray,
                CHUNK_SIZE,
            });

            worker.onmessage = (e) => {  // 当线程发送消息时的回调函数
                worker.terminate();// 关闭线程
                result[i] = e.data;  // 将线程处理的结果存储到结果数组中
                finishCount++;  // 增加完成的线程计数
                if (finishCount === numberOfThreads) {  // 如果所有线程都完成了
